...
import { IAssetsActionType } from 'models/IAssetsState';
  ...
  fetchAssetsList =
    (
      ...
      async (dispatch: Dispatch<any>) => {
        dispatch({
          type: IAssetsActionType.ASSETS_REQUEST,
        });
        await axios
          .post(
            `${baseURL}/Assets/getAllAssets`,
            {
              purchasedDate: purchasedDate ?? null,
              dataRetrievalDate: dataRetrievalDate ?? null,
              assetStatus: assetStatus ?? null, // Include assetStatus in the request payload
              manufacturerName: manufacturerName ?? null, // Include manufacturerName in the request payload
          ...
          .then((response) => {
            dispatch({
              type: IAssetsActionType.ASSETS_SUCCESS,
              payload: { data: response.data },
            });
          })
          .catch((error) => {
            dispatch({
              type: IAssetsActionType.ASSETS_FAILURE,
              payload: { msg: error },
            });
          });
      };
