...
export const ExportEmployee = ({ userDetail, Dynamiccolumn }: any) => {
  const dispatch = useDispatch();

  const empData: any = useSelector(empSelector);

  const empName = empData?.map((el: any) => el.departmentName);
  ...
  return (
    <button type="button" className={classes.root} onClick={downloadtoExcel}>
      ...
      <span style={{ fontSize: '16px' }}>Sample Report</span>
    </button>
  );
};
