...
export const ExportProject = ({ userDetail, Dynamiccolumn }: any) => {
  const dispatch = useDispatch();

  const projectData: any = useSelector(empSelector);

  const projectName = projectData?.map((el: any) => el.departmentName);
  ...
  const downloadtoExcel = async () => {
    dispatch(projectService.exportProject());
  ...
  return (
    <button type="button" className={classes.root} onClick={downloadtoExcel}>
      ...
      <span style={{ fontSize: '16px' }}>Sample Project</span>
    </button>
  );
};
